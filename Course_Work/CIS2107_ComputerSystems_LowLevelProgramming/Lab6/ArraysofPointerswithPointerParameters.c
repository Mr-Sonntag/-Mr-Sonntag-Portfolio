...
//Function Prototypes
void fillArray(int (* const gPtr)[exams]);
void userMenu (void);
void printArray(const int (* const gPtr)[exams]);
void minimum(const int (* const gPtr)[exams]);
void maximum(const int (* const gPtr)[exams]);
void average(const int (* const gPtr)[exams]);
void terminate(void);
...
void userMenu(void) {
    ...
    const int grades[students][exams];
    fillArray(grades);

    //Array of Pointers to Functions
    void (*const processGrades[4])(const int(*const)[exams]) = { printArray, minimum, maximum, average };
...
void fillArray(int (* const gPtr)[exams]) {
    for (size_t x = 0; x < students; x++) {
        for (size_t y = 0; y < exams; y++) {
            int element = 0;
            printf("\n\tEnter grade for the element in grades[%lu][%lu]: ", x, y);
            scanf("%d",&element);
            gPtr[x][y] = element;
        }
    }
